...
class MessageManager:
    ...
    def set_messages(
            self, new_messages: Dict[str, Dict[str, str]], source_class: Optional[str] = None,
        ...
        for lang, messages in new_messages.items():
            ...
            for key, message in messages.items():
                ...
                prefixed_key = f'{source_class}_{source_method}_{key}' if source_class and source_method else key
                
                if prefixed_key not in self.messages[lang]:
                    self.messages[lang][prefixed_key] = message
                else:
                    logging.warning(
                        f'Conflict for key "{prefixed_key}" from "{source_class}.{source_method}". '
                        f'Original message will be kept.')
    ...
    def get_message(self, key: str, **kwargs: Any) -> Optional[str]:
        ...
        if language_messages:
            ...
            if message_template:
                ...
                logging.error(f'Message key "{key}" not found in language "{self.default_language}"')
                # raise KeyError(f'Message key "{key}" not found in language "{self.default_language}"')
                return None
        else:
            logging.error(f'Language "{self.default_language}" is not supported')
            # raise ValueError(f'Language "{self.default_language}" is not supported')
            return None
...
if __name__ == "__main__":
    ...
    class MessageApp:
        ...
        def setup_messages(self):
            ...
            additional_messages = {
                'en': {
                    'custom_greeting': "Welcome to our application, {name}!",
                    'custom_farewell': "Thank you for using our application, {name}!",
                    # Добавлено для английского языка
                }, 'ru': {
                    'custom_farewell': "Спасибо за использование нашего приложения, {name}!",
                },
            }
            
            self._message_manager.set_messages(
                additional_messages, source_class=self._class_name, source_method=self._method_name)
